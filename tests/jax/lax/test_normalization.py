...
def test_rmsnorm_lax(shape, dtype):
    ...
    y_ref = rmsnorm_ref(x32, gamma32, eps)
    ...
    def loss_fn_ref(x32, gamma32):
        return jnp.sum(rmsnorm_ref(x32, gamma32, eps))

    grad_fn = jax.grad(loss_fn, argnums=(0, 1))
    grad_fn_ref = jax.grad(loss_fn_ref, argnums=(0, 1))

    grad_x, grad_gamma = grad_fn(x, gamma)
    grad_x_ref, grad_gamma_ref = grad_fn_ref(x32, gamma32)
